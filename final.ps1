...
# Palette feuille 1 originale (Matrice Droits)
$paletteF1 = @(
...
# Cellule A1 - diagonale
$ws.Cells.Item(1,1).Value2              = "Utilisateur | DFS"
$ws.Cells.Item(1,1).Font.Bold           = $true
$ws.Cells.Item(1,1).Font.Size           = 11
$ws.Cells.Item(1,1).Font.Color          = $noir
...
# En-tetes groupes fonctionnels - diagonale 45 degres
$colIndex  = 2
$colorIndex = 0
foreach ($groupe in $groupes) {
    ...
    $cell.Interior.Color      = $paletteF1[$colorIndex % $paletteF1.Count]
    $cell.Orientation         = 45
    $cell.HorizontalAlignment = -4108
...
foreach ($user in ($utilisateursActifs | Sort-Object SamAccountName)) {
    $couleurCase  = if ($ligneIndex % 2 -eq 0) { Get-ExcelColor 242 242 242 } else { Get-ExcelColor 225 225 225 }
    $couleurCroix = if ($ligneIndex % 2 -eq 0) { Get-ExcelColor 99 99 99 } else { Get-ExcelColor 80 80 80 }
    Write-UserRow -ws $ws -row $rowCurrent -label $user.SamAccountName `
        -couleurCase $couleurCase -couleurCroix $couleurCroix `
...
foreach ($user in ($utilisateursSuppr | Sort-Object SamAccountName)) {
    $couleurCase  = if ($ligneIndex % 2 -eq 0) { Get-ExcelColor 255 220 225 } else { Get-ExcelColor 255 245 247 }
...
foreach ($sam in ($aAjouter | Sort-Object)) {
    $couleurCase = if ($ligneIndex % 2 -eq 0) { Get-ExcelColor 210 228 245 } else { Get-ExcelColor 245 250 255 }
    Write-UserRow -ws $ws -row $rowCurrent -label $sam `
        -couleurCase $couleurCase -couleurCroix $blanc `
...
# En-tetes DFS actifs - diagonale 45
$col = 2
...
# En-tetes fonctionnels - diagonale 45
$col = 2
