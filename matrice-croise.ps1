...
# ============================================================
# CALCUL HAUTEUR LIGNE 1 AUTOMATIQUE
# On cherche le nom de groupe le plus long pour adapter la hauteur
# ============================================================
$maxLen = ($groupes | ForEach-Object { $_.Name.Length } | Measure-Object -Maximum).Maximum
# ~6 pixels par caractère en taille 10 avec rotation 90°
$hauteurLigne1 = [Math]::Max(80, $maxLen * 6)
...
$ws.Cells.Item(1,1).HorizontalAlignment = -4108  # centré
$ws.Cells.Item(1,1).VerticalAlignment   = -4108
$ws.Rows.Item(1).RowHeight = $hauteurLigne1
...
foreach ($groupe in $groupes) {
    ...
    $cell.Orientation = 90
    $cell.HorizontalAlignment = -4108  # centré horizontalement
    $cell.VerticalAlignment   = -4107  # bas (texte lisible vers le haut)
...
# Auto-ajustement largeur colonnes groupes APRÈS avoir rempli les valeurs
# On force d'abord une largeur minimale de 4, puis on laisse Excel calculer
for ($c = 2; $c -le ($nbGroupes + 1); $c++) {
    $ws.Columns.Item($c).AutoFit() | Out-Null
    # On s'assure d'un minimum de 4 et maximum de 20
    if ($ws.Columns.Item($c).ColumnWidth -lt 4)  { $ws.Columns.Item($c).ColumnWidth = 4 }
    if ($ws.Columns.Item($c).ColumnWidth -gt 20) { $ws.Columns.Item($c).ColumnWidth = 20 }
}
...
function Write-UserRow($ws, $row, $label, $couleurNom, $couleurCase, $couleurCroix, $groupes, $membresParGroupe, $samComplet) {
    $ws.Rows.Item($row).RowHeight = 14

    # Colonne nom — toujours blanc
...
# Alternance blanc / gris clair, croix gris foncé
# ============================================================
$rowCurrent = 2
$ligneIndex = 0
...
# ============================================================
# BLOC 2 — A SUPPRIMER
# Alternance rose / blanc, croix rouge foncé
# ============================================================
$rowCurrent++
Write-SeparatorRow -ws $ws -row $rowCurrent `
    -texte "⚠ A SUPPRIMER — dans l'AD mais absents des actifs" `
...
$rowCurrent++
Write-SeparatorRow -ws $ws -row $rowCurrent `
    -texte "⚠ A AJOUTER — dans les actifs mais absents de l'AD" `
...
# ============================================================
# FINITIONS — colonne utilisateur + figer volets
# ============================================================
...
# SAUVEGARDE
# ============================================================
$fichier = "C:\Users\m.merme\Downloads\matrice_droittssssssss.xlsx"
$wb.SaveAs($fichier)
$wb.Close()
$excel.Quit()
